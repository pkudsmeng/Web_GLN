...
// 范围查询中的矩形搜索区域图层
var extentSearchRectangleLayer = null

const RedDotMarker = new BMap.Icon("./png/RedDotMarker.png", new BMap.Size(32, 32), {
    // 因为图片添加到地图上，是显示的左上角的坐标
    // 所以需要将图片往左上角进行平移，保证图片中心就是标注中心
    // anchor 属性表示 定位点距离图片左上角的偏移量
...
var vue = new Vue({
    ...
    methods: {
        ...
        GridSearchByExtent () {
            let _this = this;
            // 先将范围的矩形框绘制到地图上
            this.AddExtentSearchLayer()

            // 根据业务需求，来自定义到底是定位到哪边
            //map.centerAndZoom(new BMap.Point(_this.extentSearchForm.minLon, _this.extentSearchForm.minLat), 12);
            map.centerAndZoom(new BMap.Point(100.771252, 21.957884), 12);
            ...
            this.$http.get('./data/range.json')
                .then(function (res) {
                    ...
                    // 将json的长度和耗时，记录到左侧的结果面板中
                    _this.resultForm.count = jsonData.length;
                    _this.resultForm.searchingTime = Math.random(0.003, 0.05) * 1000;

                    // 将结果中的点，绘制到地图上
                    for(let onePoint of jsonData){
                        _this.AddMarker(onePoint['LON'],onePoint['LAT'])
                    }
                })
                .catch(err => {
                        console.log(err)
        ...
        // 将范围的矩形框绘制到地图上
        AddExtentSearchLayer () {
            let _this = this
            if (extentSearchRectangleLayer != null) {
                map.removeOverlay(extentSearchRectangleLayer)
            }
            extentSearchRectangleLayer = new BMap.Polygon([
                new BMap.Point(_this.extentSearchForm.minLon, _this.extentSearchForm.minLat),
                new BMap.Point(_this.extentSearchForm.minLon, _this.extentSearchForm.maxLat),
                new BMap.Point(_this.extentSearchForm.maxLon, _this.extentSearchForm.maxLat),
                new BMap.Point(_this.extentSearchForm.maxLon, _this.extentSearchForm.minLat)
            ], {
                // 这边是矩形框的样式属性对象
                // strokeColor: "blue", // 边框颜色、粗细、矩形范围的透明度
                // strokeWeight: 2,
                strokeOpacity: 0,
                strokeWeight: 0.1,
                strokeColor:"blue",
            ...
            map.addOverlay(extentSearchRectangleLayer);
        },

        // 添加点标注
        AddMarker (lon, lat) {
            let marker = new BMap.Marker(new BMap.Point(lon, lat),{icon:RedDotMarker});
            map.addOverlay(marker);
        },
        // 删除所有的地图覆盖物
        RemoveAllOverlay(){
